...
## Reusable GitLab CI Pipelines

Reusable application CI templates are owned by the
`modules/gitlab_ci_pipelines` submodule. This keeps pipeline generation separate
from GitLab project lifecycle settings and from dynamic environment
orchestration, while leaving room for additional pipeline templates later.

Projects receive the reusable build entrypoint by default. For each managed
project where `gitlab_projects[].build_pipeline` is omitted or set to an
object value, Terraform generates `ci-pipelines/build.gitlab-ci.yml` on
`feature/build-gitlab-ci-template` and opens a merge request titled
`Add reusable build-only GitLab CI template`. Set
`build_pipeline = null` to disable build template generation for a
specific project.

Projects also receive the reusable deploy entrypoint by default. For each
managed project where `gitlab_projects[].deploy_pipeline` is omitted or set to
an object value, Terraform generates `ci-pipelines/deploy.gitlab-ci.yml` on
`feature/deploy-gitlab-ci-template` and opens a merge request titled
`Add reusable Helm deploy GitLab CI template`. Set
`deploy_pipeline = null` to disable deploy template generation for a specific
project.
...
Generated files include only one hidden `.build` template. Concrete jobs,
`stages`, `needs`, `rules`, and service-specific variables stay
consumer-managed in the root CI config or other included files. The hidden
`.build` template uses `stage: build`, `amazon/aws-cli:2.28.8`, and
`docker:dind` with Docker TLS disabled, `DOCKER_HOST=tcp://docker:2375/`,
and `DOCKER_DRIVER=overlay2`. These image and service blocks are defaults and
can be overridden per project through `build_pipeline`.

The generated `.build` template expects concrete jobs to use `extends: .build`
and pass behavior through CI variables only. Standard runtime variables include
`BUILD_MODE`, `REGISTRY_PROVIDER`, `IMAGE_REPOSITORY`, `IMAGE_TAG`,
`DOCKERFILE_PATH`, and `BUILD_CONTEXT`. Optional variable-driven hooks such as
`BUILD_PRE_SCRIPT`, `BUILD_SETUP_SCRIPT`, and `BUILD_COMMAND` are available for
repositories that need custom preparation or a custom build invocation without
rewriting the reusable `.build` job.

For ECR, provide `AWS_REGION` and AWS credentials usable by
`aws ecr get-login-password`.
For Docker Hub, provide `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`.

The generated `.deploy` template expects concrete jobs to use
`extends: .deploy` and pass behavior through CI variables only. Standard
runtime variables include `KUBE_CONTEXT`, `AWS_EKS_CLUSTER_NAME`, `AWS_REGION`,
`KUBE_NAMESPACE`, `HELM_RELEASE`, and `HELM_CHART`. Optional variables include
`HELM_CHART_VERSION`, `HELM_VALUES_ARGS`, `HELM_SET_ARGS`, `HELM_EXTRA_ARGS`,
`DEPLOY_IMAGE_REPOSITORY`, `DEPLOY_IMAGE_TAG`,
`HELM_IMAGE_REPOSITORY_SET_PATH`, and `HELM_IMAGE_TAG_SET_PATH`.
If `dynamic_environment.enabled = true`, the generated hidden `.deploy`
...
Set these variables in the concrete deploy job from the root `.gitlab-ci.yml`.

Example service job:

```yaml
include:
  - local: ci-pipelines/build.gitlab-ci.yml
  - local: ci-pipelines/deploy.gitlab-ci.yml
...
build:
  ...
  variables:
    ...
    IMAGE_REPOSITORY: $AWS_ECR_REGISTRY
...
deploy:
  extends: .deploy
  variables:
    KUBE_CONTEXT: $GITLAB_AGENT_PATH
    KUBE_NAMESPACE: my-service
    HELM_RELEASE: my-service
    HELM_CHART: dasmeta/base
    HELM_CHART_VERSION: 1.2.3
    HELM_VALUES_ARGS: -f helm/values.yaml -f helm/values.prod.yaml
    DEPLOY_IMAGE_REPOSITORY: $AWS_ECR_REGISTRY
    DEPLOY_IMAGE_TAG: $CI_COMMIT_SHORT_SHA
    HELM_IMAGE_REPOSITORY_SET_PATH: image.repository
    HELM_IMAGE_TAG_SET_PATH: image.tag
```
...
description includes this manual include snippet:
...
| <a name="output_gitlab_ci_build_pipeline_file_paths"></a> [gitlab\_ci\_build\_pipeline\_file\_paths](#output\_gitlab\_ci\_build\_pipeline\_file\_paths) | Map of service project name to generated reusable build CI file path. |
| <a name="output_gitlab_ci_build_pipeline_source_branches"></a> [gitlab\_ci\_build\_pipeline\_source\_branches](#output\_gitlab\_ci\_build\_pipeline\_source\_branches) | Map of service project name to generated reusable build CI source branch. |
| <a name="output_gitlab_ci_deploy_pipeline_file_paths"></a> [gitlab\_ci\_deploy\_pipeline\_file\_paths](#output\_gitlab\_ci\_deploy\_pipeline\_file\_paths) | Map of service project name to generated reusable deploy CI file path. |
| <a name="output_gitlab_ci_deploy_pipeline_source_branches"></a> [gitlab\_ci\_deploy\_pipeline\_source\_branches](#output\_gitlab\_ci\_deploy\_pipeline\_source\_branches) | Map of service project name to generated reusable deploy CI source branch. |
