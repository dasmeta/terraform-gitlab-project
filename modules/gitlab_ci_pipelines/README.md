...
The first supported pipeline type is `build_ecr`. It creates:

```text
ci-pipelines/build-gitlab.ci.yaml
```

The generated file includes the shared reusable build template from
`das-meta/gitlab-ci-templates` and defines a concrete `build` job that extends
`.build` with project-specific variables.
